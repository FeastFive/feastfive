...
const { doneOrder } = require("../../controllers/mealController");

const app = express();
app.use(express.json());
app.put("/api/meals", doneOrder);
...
describe("doneOrder", () => {
  ...
  // it("should mark order as done successfully", async () => {
  //   const mockRestaurant = {
  //     orders: [{ orderId: "order1", status: "In Progress", activate: true }]
  //   };
  //   const mockUser = { orders: [{ orderId: "order1", status: "In Progress", activate: true }] };
  ...
  //     .put("/api/meals")
  //     .send({ restaurantId: "restaurant1", userId: "user1", orderId: "order1" });

  //   expect(res.status).toBe(200);
  //   expect(res.body.state).toBe("success");
  //   expect(mockRestaurant.orders[0].status).toBe("Done");
  //   expect(mockRestaurant.orders[0].activate).toBe(false);
  //   expect(mockUser.orders[0].status).toBe("Done");
  //   expect(mockUser.orders[0].activate).toBe(false);
  // });
