  ...
  useEffect(() => {
    ...
    if (cookieLabelList.length > 0) {
      foods?.forEach((element) => {
        let value = 0;
        if (element && element.labels) { // Add null check for element and labels
          for (let index = 0; index < element.labels.length; index++) {
            let checkLabel = cookieLabelList.find((e) => e.value === element.labels[index].label); // Fix comparison operator
            if (checkLabel) {
